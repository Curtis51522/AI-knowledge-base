...
### S2 ????
- ??? 3 ? XGBoost quantile ?? (Q10/Q50/Q90) ? `s2_forecasting/outputs/`
- ????: ???? + ???????? + ??????? lag
- `generate_7day_s2_forecast(start_date)` ?? 7 ???
- ????: ????????????


## ?????? (`--eval`)

?? S2 8/1/1 ??? (2023-07-01 ~ 2023-12-31, 184?)??? lag ???? S3?
- 27 ?????
- Bake/Actual ratio: 0.394?S2 ?????????????
- ???: CNY 251,532.85
- ????: CNY 9,316.03
- ??: `python s3_scheduling/scheduler.py --eval --save outputs/paper_eval.json`
...
| ?? | ?? | ?? | ?? |
|------|------|------|------|
| GET | `/s3/plan/7day` | `?date=YYYY-MM-DD` | Forecasting Panel 2: 7????? |
| GET | `/s3/materials` | `?date=YYYY-MM-DD` | Forecasting Panel 3: ??????? |
| GET | `/s3/eval` | ? | ???????30s?????? |

????? `api/module3_scheduling.py`??? `main.py` ? `/s3` ?????
